...
class _S3BrowserPageState extends State<S3BrowserPage> {
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      FilledButton.icon(
                                        ...
                                        label: Text(context.loc('refresh_now')),
                                      ),
                                    ],
                                  ),
                                ))
    ...
    if (_isSelectionMode) {
      return [
        ...
        IconButton(
          ...
          tooltip: context.loc('select_all'),
        ),
        // Batch download
        IconButton(
          icon: const Icon(Icons.download),
          onPressed: _selectedItems.isEmpty ? null : _batchDownload,
          tooltip: context.loc('download_selected'),
        ),
        // Batch delete
        IconButton(
          icon: Icon(
        ...
        PopupMenuItem(
          ...
          child: Row(
            children: [
              ...
              Text(context.loc('create_folder')),
            ],
          ),
        ),
        PopupMenuItem(
    ...
    if (Platform.isMobile) {
      return [
        ...
        IconButton(
          ...
          tooltip: context.loc('select_files'),
        ),
        // View toggle button
        IconButton(
  ...
  List<MapEntry<String, String>> _mobilePathMenuItems() {
    final entries = <MapEntry<String, String>>[
      MapEntry(widget.serverConfig.name, ''),
    ];
    final parts = _currentPrefix
        .split('/')
        .where((part) => part.isNotEmpty)
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      entries.add(MapEntry(parts[i], prefix));
    }
    return entries;
  }

  Widget _buildMobileDirectoryTitle() {
    final menuItems = _mobilePathMenuItems();
    return PopupMenuButton<String>(
      position: PopupMenuPosition.under,
      tooltip: context.loc('open'),
      onSelected: (targetPrefix) {
        if (targetPrefix == _currentPrefix) return;
  ...
  List<PopupMenuEntry<String>> _buildContextMenuItems(S3Item object) {
    return [
      PopupMenuItem(value: 'download', child: Text(context.loc('download'))),
      if (!object.isDirectory) ...[
        PopupMenuItem(value: 'copy', child: Text(context.loc('copy_url'))),
        PopupMenuItem(value: 'rename', child: Text(context.loc('rename'))),
        PopupMenuItem(value: 'select', child: Text(context.loc('select'))),
      ],
